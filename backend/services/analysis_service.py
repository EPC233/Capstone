...
def analyze_csv(csv_content: str | bytes, sample_rate: int = 100, 
    ...
    rest_sensitivity : float
        Multiplier for the trim threshold used to detect rest periods
        between reps.  Higher values trim more aggressively, creating
        larger dead zones during pauses.  Range 0.1–2.0 (default 0.5).
    weight_kg : float
    ...
    for i, (rs, re) in enumerate(reps_raw):
        ...
        # Eccentric = upward (position rising from start to peak)
        ecc_rom = abs(float(position_m[peak_idx]) - float(position_m[rs]))
        eccentric = _build_phase(rs, peak_idx, ecc_rom, compute_watts=True)

        # Concentric = downward (position falling from peak to end)
        con_rom = abs(float(position_m[peak_idx]) - float(position_m[re]))
        concentric = _build_phase(peak_idx, re, con_rom, compute_watts=False)
