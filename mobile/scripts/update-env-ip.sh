...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    # Linux - try multiple methods without hostname
    IP=$(ip route get 1.1.1.1 2>/dev/null | grep -oP 'src \K[^ ]+' || ip route get 1.1.1.1 2>/dev/null | awk '{print $7; exit}' || ip addr show | grep "inet " | grep -v 127.0.0.1 | awk '{print $2}' | cut -d'/' -f1 | head -1)
else
